...
const FoodCard = (p) => {
  ...
  const stabilesteExpirare = (date) => {
    ...
    let diffDays = Math.ceil(date - currentDate / (1000 * 60 * 60 * 24));
    if (diffDays < 4) {
  ...
  const rezervaProdus = (prop) => {
    try {
      ...
      newProduct.status = 0;
      axios
        .put("http://localhost:3030/api/products/" + newProduct.id, newProduct)
        .then(() => {
          prop.stateModified();
        });
      ...
      axios
        .post("http://localhost:3030/api/reservations/", obj)
        .then(() => {})
        .catch((err) => {
          console.log(err);
        });
  ...
  return (
    <div className="foodCard">
      <div className={`reservedStatus ${statusList[p.product.status]}`}>
        {p.product.category +
          " - " +
          statusList[p.product.status][0].toUpperCase() +
          statusList[p.product.status].substring(1)}
      </div>
      <img
        src={
          p.product.photoURL !== "Empty"
            ? p.product.photoURL
      ...
      <p className="textContent">
        Quantity: {p.quantity.units}, {p.quantity.type}
      </p>
      <p className={stabilesteExpirare(Date.parse(p.product.expDate))}>
        Expiration date: {p.product.expDate.substring(0, 10)}
      </p>
      <p className="textContent">Location: {p.product.address}</p>
      <p className="textContent"> Phone: {p.user.phone}</p>
      <div
        className="sellingContent"
        style={{
          visibility:
            (p.product.status === ProductStatus.RESERVED ||
              p.product.status === ProductStatus.SOLD) &&
            auth.currentUser.uid === p.product.idUser
              ? "visible"
              : "hidden",
        }}
      >
        ...
        <input
          ...
          checked={p.product.status == ProductStatus.SOLD ? true : false}
        ></input>
      </div>
      <Tippy
        content={setButtonTooltipMessage(
